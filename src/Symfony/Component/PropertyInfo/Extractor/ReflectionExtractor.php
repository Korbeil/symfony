...
class ReflectionExtractor implements PropertyListExtractorInterface, PropertyTypeExtractorInterface, PropertyAccessExtractorInterface, PropertyInitializableExtractorInterface, PropertyReadInfoExtractorInterface, PropertyWriteInfoExtractorInterface, ConstructorArgumentTypeExtractorInterface
{
    ...
    public function __construct(array $mutatorPrefixes = null, array $accessorPrefixes = null, array $arrayMutatorPrefixes = null, bool $enableConstructorExtraction = true, int $accessFlags = self::ALLOW_PUBLIC, InflectorInterface $inflector = null, int $magicMethodsFlags = self::ALLOW_MAGIC_GET | self::ALLOW_MAGIC_SET)
    {
        ...
        $this->typeResolver = new ChainTypeResolver([
    ...
    public function getType(string $class, string $property, array $context = []): ?Type
    {
        ...
        if ($fromAccessor = $this->extractFromAccessor($class, $property, $backwardCompatible)) {
            return $fromAccessor;
        }
    ...
    public function getTypes(string $class, string $property, array $context = []): ?array
    {
        ...
        return BackwardCompatibilityHelper::convertTypeToLegacyTypes($this->getType($class, $property, $context, true), keepNullType: false);
    }
    ...
    private function extractFromMutator(string $class, string $property): ?Type
    {
        ...
        try {
            $type = $this->typeResolver->resolve($reflectionMethod->getParameters()[0]);
        } catch (UnsupportedException) {
            return null;
        }
     ...
     */
    private function extractFromAccessor(string $class, string $property): ?Type
    {
        ...
        try {
            ...
            if ('void' === (string) $reflectionMethod->getReturnType() && $backwardCompatible) {
     ...
     */
    private function extractFromConstructor(string $class, string $property): ?Type
    {
        try {
            $reflectionClass = new \ReflectionClass($class);
        } catch (\ReflectionException) {
    ...
    private function extractFromPropertyDeclaration(string $class, string $property): ?Type
    {
        try {
            $reflectionClass = new \ReflectionClass($class);
        ...
        $type = 'array' === $type ? Type::array() : Type::builtin($type);
