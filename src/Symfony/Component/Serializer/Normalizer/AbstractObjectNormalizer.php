...
abstract class AbstractObjectNormalizer extends AbstractNormalizer
{
    ...
    private function validateAndDenormalize(Type $type, string $currentClass, string $attribute, mixed $data, ?string $format, array $context): mixed
    {
        ...
        if ($type instanceof UnionType && $nullable) {
        ...
        $types = match (true) {
            ...
            $type instanceof IntersectionType => $type->getTypes(),
            default => [$type],
        };
        ...
        foreach ($types as $t) {
            ...
            $collectionValueTypeNullable = false;
            ...
            if ($t instanceof CollectionType) {
                ...
                if ($collectionValueType instanceof UnionType) {
                    $collectionValueTypeNullable = true;
                    $collectionValueType = $collectionValueType->asNonNullable();
                }
            ...
            if ('xml' === $format && $collectionValueType && !$collectionValueType->isBuiltinType(BuiltinTypeEnum::Mixed) && (!\is_array($data) || !\is_int(key($data)))) {
                $data = [$data];
            }
            ...
            try {
                $builtinType = $t->getBuiltinType()->value;
                ...
                if (\is_string($data) && (XmlEncoder::FORMAT === $format || CsvEncoder::FORMAT === $format)) {
                    if ('' === $data) {
                        ...
                        if ($nullable && \in_array($builtinType, ['bool', 'int', 'float'], true)) {
                            return null;
                        }
                    }
                    ...
                    switch ($builtinType) {
                        case 'bool':
                            ...
                            if ('false' === $data || '0' === $data) {
                                ...
                                throw NotNormalizableValueException::createForUnexpectedDataType(sprintf('The type of the "%s" attribute for class "%s" must be bool ("%s" given).', $attribute, $currentClass, $data), $data, ['bool'], $context['deserialization_path'] ?? null);
                            }
                            break;
                        case 'int':
                            if (ctype_digit('-' === $data[0] ? substr($data, 1) : $data)) {
                                ...
                                throw NotNormalizableValueException::createForUnexpectedDataType(sprintf('The type of the "%s" attribute for class "%s" must be int ("%s" given).', $attribute, $currentClass, $data), $data, ['int'], $context['deserialization_path'] ?? null);
                            }
                            break;
                        case 'float':
                            if (is_numeric($data)) {
                            ...
                            return match ($data) {
                                ...
                                'INF' => \INF,
                                '-INF' => -\INF,
                                default => throw NotNormalizableValueException::createForUnexpectedDataType(sprintf('The type of the "%s" attribute for class "%s" must be float ("%s" given).', $attribute, $currentClass, $data), $data, ['float'], $context['deserialization_path'] ?? null),
                            };
                    }
                }

                if ($collectionValueType && $collectionValueType->getBaseType() instanceof ObjectType) {
                    $builtinType = 'object';
                    $class = $collectionValueType->getClassName().'[]';
                    $context['key_type'] = $collectionKeyType;
                    $context['value_type'] = $collectionValueTypeNullable ? Type::nullable($collectionValueType) : $collectionValueType;
                } elseif ($collectionValueType && BuiltinTypeEnum::Array === $collectionValueType->getBaseType()->getBuiltinType()) {
                ...
                $expectedTypes['object' === $builtinType && $class ? $class : $builtinType] = true;
                ...
                // a float is expected.
                if ('float' === $builtinType && \is_int($data) && null !== $format && str_contains($format, JsonEncoder::FORMAT)) {
                    return (float) $data;
                }

                if (('false' === $builtinType && false === $data) || ('true' === $builtinType && true === $data)) {
                    return $data;
                }
