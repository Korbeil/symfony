...
final class PropertyInfoLoader implements LoaderInterface
{
    ...
    private function getTypeConstraint(TypeInfoType $type): ?Type
    {
        if ($type instanceof UnionType || $type instanceof IntersectionType) {
            return ($type->is(fn (TypeInfoType $t): bool => \in_array((string) $t, ['int', 'float', 'string', 'bool'], true)))
        ...
        if (BuiltinTypeEnum::Mixed !== $baseType->getBuiltinType()) {
            return new Type(['type' => $baseType->getBuiltinType()->value]);
        }
